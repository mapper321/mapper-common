...
//	}
	 ...
	 * @return
	 */
	public static String formatSpeacialChar(String str){
		if(str != null && !"".equals(str)){
			StringBuffer sb = new StringBuffer();
			String filename = str;
			...
			for(int i = 0; i < chars.length; i ++) { 
				if(chars[i] == 35 || (chars[i] >= 19968 && chars[i] <= 40869) || (chars[i] >= 97 && chars[i] <= 122) || (chars[i] >= 65 && chars[i] <= 90) || Character.isDigit(chars[i])) {
					sb.append(chars[i]);
				}  
			} 
			return sb.toString();
